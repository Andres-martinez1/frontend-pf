...
export default function CardFiltroE() {
    ...
    <div className="p-4">
      <CustomCard className="p-6 bg-white shadow-md rounded-xl border border-gray-200" conten={""}>
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            <h2 className="text-lg font-bold text-gray-900">
              Filtros Progresivos Inteligentes
            </h2>
            <p className="text-sm text-gray-500">
              Configure su análisis paso a paso para obtener insights precisos
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
          ...
          <div>
            <div className="flex items-center gap-2 mb-1">
              <span className="w-6 h-6 flex items-center justify-center rounded-full bg-gray-100 text-gray-500 text-xs font-bold">
                2
              </span>
              <span className="text-sm font-medium text-gray-700">
                Tipo de Análisis
          ...
          <div>
            ...
            <CustomSelect
              ...
              onChange={(val: string) => setEstado(val)}
              disabled={!tipo} 
              selectionMode={"single"}
            />
          </div>
          ...
          <div className="flex flex-wrap gap-2 mb-4">
            ...
            {tipo && (
              <span className="px-3 py-1 text-sm bg-green-100 text-green-600 rounded-full">
                {tiposAnalisis.find((t) => t.value === tipo)?.label}
              </span>
            )}
            {estado && (
              <span className="px-3 py-1 text-sm bg-purple-100 text-purple-600 rounded-full">
                {estadosEspecificos[tipo!].find((e) => e.value === estado)?.label}
              </span>
            )}
          </div>
        )}
